    ...
    if (categories[0]) {
                ...
                if (checked[i]) {
                    for (var j = i+1; j < checked.length; j++) {
                        if (checked[j]) {
                            ...
                            let result = testStringDifferences_Module.p_all(item1, item2, 3, 30)
        ...
        return {
            run: (function (category) {
                return checkDoubleLinks(category)
            }),
            p_getContentItems: (function (row) {
                return getContentItems(row)
            })
        }
    }
